...
def f(x):
    func = x * math.sin(6 * math.exp(-x**2))
    return func
...
if __name__ == '__main__':  
    ...
    c = (1/(2 * math.pi)) * trapz(f, a, b, subintervalos)
    coeffs.append(c)

    for m in range(1, 6):
        am = (1/math.pi) * trapz(lambda x: f(x) * math.cos(m * x), a, b, subintervalos)
        coeffs.append(am)
        bm = (1/math.pi) * trapz(lambda x: f(x) * math.sin(m * x), a, b, subintervalos)
        coeffs.append(bm)
    ...
    def g(x, coeffs):
        ...
        for i in range(1, len(coeffs), 2):
            soma += coeffs[i] * math.cos(math.ceil(i/2)*x)
            soma += coeffs[i+1] * math.sin(math.ceil(i/2)*x)
        return soma
